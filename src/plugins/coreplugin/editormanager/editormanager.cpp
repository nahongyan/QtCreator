...
/*!
在指定的外部编辑器中打开指定的文件名指定的文档editorId。

返回 false，如果editorId不是ID，则显示错误消息的外部编辑器，或无法打开外部编辑器
...
/*!
向被询问编辑器是否可以关闭的钩子添加一个侦听器。当编辑器请求关闭时，将调用所有侦听器。
如果其中一个调用返回 false，进程将中止，事件将被忽略。如果所有调用都返回 true，
则发出editorAboutToClose()并接受事件。
*/
