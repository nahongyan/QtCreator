...
void Locator::updateEditorManagerPlaceholderText()
{
    Command *openCommand = ActionManager::command(Constants::OPEN);
    Command *locateCommand = ActionManager::command(Constants::LOCATE);
    const QString placeholderText = tr("<html><body style=\"color:#909090; font-size:18px\">"
          "<div align='center'>"
          "<div style=\"font-size:25px\">打开文件</div>"
          "<table><tr><td>"
          "<hr/>"
          "<div style=\"margin-top: 5px\">&bull; File > 打开文件或项目 (%1)</div>"
          "<div style=\"margin-top: 5px\">&bull; File > 最近的文件</div>"
          "%4"
          "%5"
          "<div style=\"margin-top: 5px\">&bull; 拖放文件到这里</div>"
          "</td></tr></table>"
          "</div>"
          "</body></html>")
...
/*!
这将返回所有筛选器的一个子集，该子集只包含以下筛选器已由用户在某个时间点(可能在前一个会话中)创建。
...
void Locator::setFilters(QList<ILocatorFilter *> f)
{
    m_filters = f;
    updateFilterActions();
    updateEditorManagerPlaceholderText(); // 可能是快捷键变了
    emit filtersChanged();
}
