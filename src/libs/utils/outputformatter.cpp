...
//重定向机制需要被调用的坏的构建工具(例如xcodebuild)
//间接作为构建过程的一部分，并重定向其子进程的stderr输出
//发送到stdout。解析器可能会检测到这种情况并提供相关信息
//其他需要将stdout数据解释为stderr的解析器。
...
void OutputLineParser::dropSearchDir(const FilePath &dir)
{
    ...
    // TODO: 这显然触发器。找出原因并删除断言(如果它是合法的)或解决问题。
