...
/*!
    ...
    \brief PathChooser类是一个允许用户选择路径的控件。控件由QLineEdit和“浏览”按钮组成，是可选的
能够执行变量替换。这个类有一些用于嵌入到QWizardPage中的验证逻辑。
*/
...
//BinaryVersionToolTipEventFilter事件过滤器被安装在用于输入的lineedit上可执行文件，获取用于打印版本的参数('——version')。
//在工具提示事件中，版本是通过运行二进制文件和获得的将其标准输出设置为工具提示。
...
//扩展BinaryVersionToolTipEventFilter以前置现有的路径选择器,显示完整路径的工具提示。
...
{
    //因为它是我们的focusProxy，所以它可以接收焦距并发出信号
    //即使可能的祖先-接受者处于销毁之中。
