...
/*!
    ...
    \brief PersistentSettingsReader类读取任意的QVariantMap，
来自XML文件的嵌套数据结构。处理所有字符串序列化的简单类型和QVariantList和QVariantMap。例子:
...
在解析结构时，将为每个结构使用ParseValueStackEntry的解析堆栈<数据>元素。ParseValueStackEntry是:
...
您可以通过在Qt元数据中注册自定义类型的字符串序列化函数类型系统。例子:
...
当输入一个值元素(<value> /  <valuelist>，  <valuemap>)时，条目被推入
相应的行动。当离开元素时，条目的qvariable -值将从堆栈中删除
并添加到下面的堆栈条目中(添加到列表或插入到映射中)。第一个元素
的值为<data>元素的值。
...
struct Context // 包含元素名称、字符串常量的基本上下文。
{
    Context() {}
