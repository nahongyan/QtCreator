...
/*!
    ...
    \inmodule QtCreator
    \brief QrcParser类解析一个或多个QRC文件，并缓存它们的内容。
一个Qt资源系统{Qt资源集合(QRC)}包含从文件系统中读取但组织方式可能不同的文件。
为了用简单的结构方便地描述它，我们使用了从QRC路径到文件系统中的路径的映射。
通过使用map，我们可以很容易地找到所有以给定前缀开始的QRC路径，从而在QRC目录上循环。
QRC文件还支持语言，这些语言被映射到QRC路径的一个前缀。例如，
French /image/bla.png (lang=fr)的路径为{fr/image/bla.png}。空语言表示默认资源。
对于单个QRC，使用locale uiLanguages()属性查找语言，给定的路径映射到单个文件，
但是当一个文件有多个(特定于平台且相互排斥)QRC文件时，多个文件会匹配，因此使用QStringList。
特别是，collect*函数被认为是底层接口。
 */
...
/*!
通过删除QRC:/来规范化指向QRC资源中文件的路径或:以及开头的任何额外斜杠。
 */
