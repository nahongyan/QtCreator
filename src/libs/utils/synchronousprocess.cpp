...
/*!
    ...
    \brief SynchronousProcess类在它自己的进程中运行一个同步进程
只阻塞用户输入事件的事件循环。因此，它允许GUI重新绘制并将输出追加到日志窗口。


    The stdOut(), stdErr() 信号的发射没有缓冲 作为对进程写入

stdOutBuffered()、stdErrBuffered()信号由complete发出如果它们被启用，则基于“\”标记的行
stdOutBufferedSignalsEnabled () / setStdErrBufferedSignalsEnabled()。它们通常用于日志窗口。

有一个超时处理，在最后一个数据完成后生效
从stdout/stdin读取(与waitForFinished()相反，后者度量时间
因为它被调用了)。因此，它也适用于连续的慢进程输出数据(如版本系统操作)。

属性timeOutMessageBoxEnabled影响消息框是否为
显示询问用户是否想在超时时终止进程(默认:false)。
还有用于处理完全同步的静态实用程序函数读取带有正确超时处理的输出。

警告:如果有可能，这个类不应该被使用触发打开对话框(例如，由文件监视触发),因为这将导致事件循环问题。
