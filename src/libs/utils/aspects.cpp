...
/*!
    ...
    \brief BaseAspect类为实现方面的类提供了一个公共基。方面是一堆数据，比如某个对象的属性或相关属性的集合，
以及对其用于常见操作(如可视化或持久化)的行为的描述。例如，简单方面是由用户界面中的QCheckBox表示的布尔属性，
或者由选择文件系统中的目录的路径选择器表示的字符串属性。虽然aspect实现通常具有可视化和持久化数据的能力，
或者使用ID，但这两种功能都不是强制性的。
*/
