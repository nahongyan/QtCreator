...
/*!
    ...
    \brief ProjectIntroPage类是项目的标准向导页面，让用户选择它的名称和路径。
看起来类似于FileWizardPage，但是提供了额外的功能:

    \list
    \li 在顶部包含一个描述标签，用于显示介绍性文本。
    \li 是否在动态验证(连接到changed())和显示底部状态标签中的警告和错误(该页已完成
当完全验证时，validatePage()因此不会实现)。
    \endlist

    \note 在更改projectintropage.ui时要小心。它必须有几何体清除和
QLayout::SetMinimumSize限制上的主布局，否则，QWizard将挤压它由于其奇怪的扩展。
*/
