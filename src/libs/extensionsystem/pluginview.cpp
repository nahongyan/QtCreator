...
PluginView类实现了一个显示所有列表的小部件插件及其状态。
这个类可以嵌入例如应用程序中的对话框中使用插件管理器。
该类还提供与列表交互的通知。
...
public:
    PluginSpec *m_spec; // 不是所有。
    PluginView *m_view; // 不是所有。
};
