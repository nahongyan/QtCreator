...
#使用一个for循环，遍历SUBDIRS中定义的每一个目录，将QTC_LIB_DEPENDS赋值为$$l/$${l}_dependencies.pri的内容，
#该内容是由include函数引入的。如果此时的l为aggregation，
#那么，QTC_LIB_DEPENDS就会是include(aggregation/aggregation_dependencies.pri)的输出值。
for(l, SUBDIRS) {
    QTC_LIB_DEPENDS =
